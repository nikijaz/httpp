...
class Socket {
   public:
     ...
     * Returns true on success, false on failure (check errno for details).
     */
    bool connect(const sockaddr_in& addr) noexcept;
    ...
    /*
     * Resolve a hostname and port to a sockaddr_in structure.
     * Throws `std::runtime_error` on failure.
     */
    static sockaddr_in resolve_address(const std::string& hostname, uint16_t port);
