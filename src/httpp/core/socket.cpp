...
bool Socket::connect(const sockaddr_in& addr) noexcept {
    return ::connect(fd_, reinterpret_cast<const sockaddr*>(&addr), sizeof(addr)) != -1;
}
...
sockaddr_in Socket::resolve_address(const std::string& hostname, uint16_t port) {
    addrinfo hints{};
    ...
    sockaddr_in addr{};
    std::memcpy(&addr, result->ai_addr, sizeof(sockaddr_in));
